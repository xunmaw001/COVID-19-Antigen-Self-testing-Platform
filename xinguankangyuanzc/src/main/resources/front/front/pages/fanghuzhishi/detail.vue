			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.fanghuzhishiName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
					...
					<view class="yinyuenei uni-padding-wrap uni-common-mt">
						<view>
							<video id="myVideo" style="preload:auto;margin: 20rpx;border-radius: 20rpx;" muted
								   :src="baseUrl+detail.fanghuzhishiVideo" controls></video>
						</view>
					</view>
				</view>
				<!-- 简介 -->
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.fanghuzhishiContent"></rich-text>
						</view>
					</view>
				</view>
            <!-- 留言 -->
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.fanghuzhishiId = options.id;
			},
			async onShow(options) {
				// 渲染数据
				this.init();
				//获取收藏信息
				this.getfanghuzhishiCollection()
			...
			methods: {
                ...
                async submitData() {
                    ...
                    let data = {
                        fanghuzhishiId: this.fanghuzhishiId,
                        yonghuId: this.user.yonghuId,
						fanghuzhishiLiuyanPingfenNumber: this.scoreData,
                        fanghuzhishiLiuyanText: this.commentData,
                    }
                    await this.$api.save(`fanghuzhishiLiuyan`, data);
                    this.$refs.deliver.close()
                    this.mescroll.num = 1
                    this.upCallback(this.mescroll)
                },
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('fanghuzhishi', this.fanghuzhishiId);
                    this.detail = res.data
					this.detail.fanghuzhishiContent = this.detail.fanghuzhishiContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.fanghuzhishiPhoto ? this.detail.fanghuzhishiPhoto.split(",") : [];
				},
				/* && !nameList?seq_contains('zuowei_number')*/
				// 收藏
				async getfanghuzhishiCollection() {
					let params = {
						page: 1,
						limit: 1,
						fanghuzhishiId: this.fanghuzhishiId,
				...
				async shoucang() {
					...
					let res = await _this.$api.list(`fanghuzhishiCollection`, params);
					if (res.data.list.length == 1) {
						let fanghuzhishiCollectionId = res.data.list[0].id;
						uni.showModal({
							title: '提示',
							content: '是否取消',
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('fanghuzhishiCollection', JSON.stringify([fanghuzhishiCollectionId]));
									_this.$utils.msg('取消成功');
									_this.getfanghuzhishiCollection();
								}
							}
						});
						return;
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('fanghuzhishiCollection', {
									yonghuId: _this.user.id,
									fanghuzhishiId: _this.detail.id,
									fanghuzhishiCollectionTypes:1
								});
				...
				async upCallback(mescroll) {
					let res = await this.$api.list('fanghuzhishiLiuyan', {
						page: mescroll.num,
						limit: mescroll.size,
						fanghuzhishiId: this.fanghuzhishiId
					});
						...
						uni.showModal({
							...
							success: async function(res) {
								if (res.confirm) {
									...
									await _this.$api.update('fanghuzhishi', _this.detail);
									_this.$utils.msg('取消成功');
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('fanghuzhishiYuyue', {
									yonghuId: _this.user.id,
									fanghuzhishiId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/fanghuzhishiYuyue/list');
							}
						}
					});
				},
