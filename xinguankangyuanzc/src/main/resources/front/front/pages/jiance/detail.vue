	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('jiance', this.jianceId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.jiancePhoto ? this.detail.jiancePhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('jianceYuyue', {
									yonghuId: _this.user.id,
									jianceId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/jianceYuyue/list');
							}
						}
					});
				},
