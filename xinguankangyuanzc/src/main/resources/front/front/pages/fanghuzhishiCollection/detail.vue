				...
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
					<view class="xiangqing">
						<view class="shangpinxiangqing">
							...
							<view class="app">
								{{detail.fanghuzhishiCollectionValue}}
							</view>
						</view>
					</view>
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.fanghuzhishiCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('fanghuzhishiCollection', this.fanghuzhishiCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.fanghuzhishiCollectionPhoto ? this.detail.fanghuzhishiCollectionPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('fanghuzhishiCollectionYuyue', {
									yonghuId: _this.user.id,
									fanghuzhishiCollectionId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/fanghuzhishiCollectionYuyue/list');
							}
						}
					});
				},
