	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.xinguankangyuanLiuyanId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('xinguankangyuanLiuyan', this.xinguankangyuanLiuyanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.xinguankangyuanLiuyanPhoto ? this.detail.xinguankangyuanLiuyanPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('xinguankangyuanLiuyanYuyue', {
									yonghuId: _this.user.id,
									xinguankangyuanLiuyanId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/xinguankangyuanLiuyanYuyue/list');
							}
						}
					});
				},
