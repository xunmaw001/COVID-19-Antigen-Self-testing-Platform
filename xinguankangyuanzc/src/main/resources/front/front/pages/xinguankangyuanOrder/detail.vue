	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.xinguankangyuanOrderId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('xinguankangyuanOrder', this.xinguankangyuanOrderId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.xinguankangyuanOrderPhoto ? this.detail.xinguankangyuanOrderPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('xinguankangyuanOrderYuyue', {
									yonghuId: _this.user.id,
									xinguankangyuanOrderId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/xinguankangyuanOrderYuyue/list');
							}
						}
					});
				},
