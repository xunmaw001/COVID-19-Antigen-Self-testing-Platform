	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.fanghuzhishiLiuyanId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('fanghuzhishiLiuyan', this.fanghuzhishiLiuyanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.fanghuzhishiLiuyanPhoto ? this.detail.fanghuzhishiLiuyanPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('fanghuzhishiLiuyanYuyue', {
									yonghuId: _this.user.id,
									fanghuzhishiLiuyanId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/fanghuzhishiLiuyanYuyue/list');
							}
						}
					});
				},
