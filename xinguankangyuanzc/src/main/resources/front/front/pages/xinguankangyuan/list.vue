	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"2rpx 2rpx 2rpx 8rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.xinguankangyuanName" type="text" placeholder="新冠抗原名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('xinguankangyuan','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('xinguankangyuan','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='xinguankangyuanTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in xinguankangyuanTypesList" :key="index" :class="xinguankangyuanTypes===item.codeIndex?'tabActive':''"
                                  @tap="xinguankangyuanTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
    ...
    <block v-for="(product,index) in list" :key="index">
        <view v-if="index%6==0" @tap="onDetailTap(product)" class="list-item"
            ...
            <image
                    ...
                    mode="aspectFill" :src="product.xinguankangyuanPhoto?baseUrl+product.xinguankangyuanPhoto.split(',')[0]:''">
            </image>
            <view class="list-item-body" style="display: flex;flex-wrap: wrap;">
                <view
                        ...
                        class="title hide1">{{product.xinguankangyuanName}}
                </view>
                <view v-if="product.xinguankangyuanNewMoney"
                      ...
                      class="title hide1">￥{{product.xinguankangyuanNewMoney}}
                </view>
                <view v-else-if="product.xinguankangyuanValue"
                      :style='{"padding":"0 24rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(163, 169, 228, 0)","color":"#333","textAlign":"center","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"36rpx","fontSize":"28rpx","borderStyle":"solid"}'
                      class="title hide1">{{product.xinguankangyuanValue}}
                </view>
            </view>
            <view style="display: flex;justify-content: space-between;">
                <text v-if="isAuth('xinguankangyuan','修改')" class="cuIcon-edit"
                      @click.stop="onUpdateTap(product.id)">修改
                </text>
                <text v-if="isAuth('xinguankangyuan','删除')" class="cuIcon-delete"
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
                ...
                this.xinguankangyuanTypesSelect()
		...
		methods: {
            ...
            //当前表的 新冠抗原类型 字段 字典表查询方法
            async xinguankangyuanTypesSelect(){
                let xinguankangyuanTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'xinguankangyuan_types',
                });
                xinguankangyuanTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.xinguankangyuanTypesList = xinguankangyuanTypes.data.list
            },
            //当前表的 新冠抗原类型字段类别搜索
			xinguankangyuanTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.xinguankangyuanTypes != 0) {
                    params.xinguankangyuanTypes = this.xinguankangyuanTypes
                }
                let _this = this
                let res = await this.$api.list(`xinguankangyuan`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('xinguankangyuan', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.xinguankangyuanShenheTime=shijian;
                            await _this.$api.update(`xinguankangyuan`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.xinguankangyuanName) {
                    searchForm['xinguankangyuanName'] = this.searchForm.xinguankangyuanName
                }

                let res = await this.$api.list(`xinguankangyuan`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.xinguankangyuanContent!=null){
                        item.xinguankangyuanContent = item.xinguankangyuanContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
