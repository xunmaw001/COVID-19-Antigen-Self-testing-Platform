		...
		<view class="">
			<view class="price ">
				...
				<view class="pricejiner"
					  :style="detail.xinguankangyuanNewMoney>1000? 'width: 450rpx;':detail.xinguankangyuanNewMoney>10000? 'width: 550rpx;':'width: 400rpx;'">
					<text style='font-size:20rpx'>¥</text>
					<text style="font-weight:bold;font-size:50rpx">{{detail.xinguankangyuanNewMoney}}</text>
						<text style="font-size: 20rpx;color: #fff;margin-left: 10rpx;">
							<text v-if="detail.xinguankangyuanOldMoney!=null">原价￥
								<text style="text-decoration:line-through">{{detail.xinguankangyuanOldMoney}}</text>
							</text>
						</text>
				</view>
			</view>
			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.xinguankangyuanName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
				<view class="xiangqing">
					<view class="shangpinxiangqing">
						...
						<view class="app">
							{{detail.xinguankangyuanUuidNumber}}
						</view>
					</view>
				</view>
					<view class="xiangqing">
					...
					<view class="xiangqing">
						<view class="shangpinxiangqing">
							...
							<view class="app">
								{{detail.xinguankangyuanKucunNumber}}
							</view>
						</view>
					</view>
					<view class="xiangqing">
						<view class="shangpinxiangqing">
							...
							<view class="app">
								{{detail.xinguankangyuanClicknum}}
							</view>
						</view>
					</view>
				...
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.xinguankangyuanContent"></rich-text>
						</view>
					</view>
				</view>
            <!-- 留言 -->
						...
						<view v-if="commentList.length>0" v-for="(item,index) in commentList" class=""
							...
							<view v-bind:key="index" class="" style="align-items: center;">
								...
								<view style="width: 80%;margin-left: 100rpx;">
									<!-- 留言 -->
									{{item.xinguankangyuanLiuyanText}}
								</view>
							</view>
							<!-- 回复 -->
				...
				<view class="bottom-content cu-bar bg-white tabbar border shop">
					<view style="width: 100%;margin: 0 auto;display: flex;align-items: center;justify-content: space-around;">
						...
						<view @tap="onSubmit" style="display: flex; width: 70%;color: #FFFFFF;">
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.xinguankangyuanId = options.id;
			},
			async onShow(options) {
				// 渲染数据
				this.init();
				//获取收藏信息
				this.getxinguankangyuanCollection()
			...
			methods: {
                ...
                async submitData() {
                    ...
                    let data = {
                        xinguankangyuanId: this.xinguankangyuanId,
                        yonghuId: this.user.yonghuId,
						xinguankangyuanLiuyanPingfenNumber: this.scoreData,
                        xinguankangyuanLiuyanText: this.commentData,
                    }
                    await this.$api.save(`xinguankangyuanLiuyan`, data);
                    this.$refs.deliver.close()
                    this.mescroll.num = 1
                    this.upCallback(this.mescroll)
                },
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('xinguankangyuan', this.xinguankangyuanId);
                    this.detail = res.data
					this.detail.xinguankangyuanContent = this.detail.xinguankangyuanContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.xinguankangyuanPhoto ? this.detail.xinguankangyuanPhoto.split(",") : [];
				},
				...
				// 收藏
				async getxinguankangyuanCollection() {
					let params = {
						page: 1,
						limit: 1,
						xinguankangyuanId: this.xinguankangyuanId,
					...
					let res = await this.$api.page(`xinguankangyuanCollection`, params);
				...
				async shoucang() {
					...
					let res = await _this.$api.list(`xinguankangyuanCollection`, params);
					if (res.data.list.length == 1) {
						let xinguankangyuanCollectionId = res.data.list[0].id;
						uni.showModal({
							title: '提示',
							content: '是否取消',
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('xinguankangyuanCollection', JSON.stringify([xinguankangyuanCollectionId]));
									_this.$utils.msg('取消成功');
									_this.getxinguankangyuanCollection();
								}
							}
						});
						return;
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('xinguankangyuanCollection', {
									yonghuId: _this.user.id,
									xinguankangyuanId: _this.detail.id,
									xinguankangyuanCollectionTypes:1
								});
				...
				async upCallback(mescroll) {
					let res = await this.$api.list('xinguankangyuanLiuyan', {
						page: mescroll.num,
						limit: mescroll.size,
						xinguankangyuanId: this.xinguankangyuanId
					});
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('xinguankangyuanYuyue', {
									yonghuId: _this.user.id,
									xinguankangyuanId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/xinguankangyuanYuyue/list');
							}
						}
					});
				},
