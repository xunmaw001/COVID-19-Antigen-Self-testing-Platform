				...
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
					<view class="xiangqing">
						<view class="shangpinxiangqing">
							...
							<view class="app">
								{{detail.xinguankangyuanCollectionValue}}
							</view>
						</view>
					</view>
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.xinguankangyuanCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('xinguankangyuanCollection', this.xinguankangyuanCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.xinguankangyuanCollectionPhoto ? this.detail.xinguankangyuanCollectionPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('xinguankangyuanCollectionYuyue', {
									yonghuId: _this.user.id,
									xinguankangyuanCollectionId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/xinguankangyuanCollectionYuyue/list');
							}
						}
					});
				},
