...
public class JianceController {
    ...
    private static final String TABLE_NAME = "jiance";
    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(jiance !=null){
            ...
            //级联表 用户
            //级联表
            YonghuEntity yonghu = yonghuService.selectById(jiance.getYonghuId());
            if(yonghu != null){
    ...
    */
    ...
    public R save(@RequestBody JianceEntity jiance, HttpServletRequest request){
        ...
        if(false)
            ...
            jiance.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<JianceEntity> queryWrapper = new EntityWrapper<JianceEntity>()
            .eq("yonghu_id", jiance.getYonghuId())
            .eq("jiance_name", jiance.getJianceName())
    ...
    */
    ...
    public R update(@RequestBody JianceEntity jiance, HttpServletRequest request) throws NoSuchFieldException, ClassNotFoundException, IllegalAccessException, InstantiationException {
        logger.debug("update方法:,,Controller:{},,jiance:{}",this.getClass().getName(),jiance.toString());
        JianceEntity oldJianceEntity = jianceService.selectById(jiance.getId());//查询原先数据
...
//        else if("用户".equals(role))
//            jiance.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        if("".equals(jiance.getJiancePhoto()) || "null".equals(jiance.getJiancePhoto())){
                jiance.setJiancePhoto(null);
        }
    ...
    */
    ...
    public R delete(@RequestBody Integer[] ids, HttpServletRequest request){
        ...
        List<JianceEntity> oldJianceList =jianceService.selectBatchIds(Arrays.asList(ids));//要删除的数据
    ...
    public R save( String fileName, HttpServletRequest request){
        ...
        try {
            List<JianceEntity> jianceList = new ArrayList<>();//上传的东西
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            JianceEntity jianceEntity = new JianceEntity();
//                            jianceEntity.setYonghuId(Integer.valueOf(data.get(0)));   //用户 要改的
//                            jianceEntity.setJianceName(data.get(0));                    //标题 要改的
//                            jianceEntity.setJiancePhoto("");//详情和图片
//                            jianceEntity.setJianceTypes(Integer.valueOf(data.get(0)));   //检测结果 要改的
//                            jianceEntity.setJianceTime(sdf.parse(data.get(0)));          //检测时间 要改的
//                            jianceEntity.setInsertTime(date);//时间
//                            jianceEntity.setJianceText(data.get(0));                    //备注 要改的
//                            jianceEntity.setCreateTime(date);//时间
                            jianceList.add(jianceEntity);
                        ...
                        //查询是否重复
                        jianceService.insertBatch(jianceList);
                        return R.ok();
                    }
                }
            }
    ...
    */
    @RequestMapping("/add")
    public R add(@RequestBody JianceEntity jiance, HttpServletRequest request){
        logger.debug("add方法:,,Controller:{},,jiance:{}",this.getClass().getName(),jiance.toString());
        Wrapper<JianceEntity> queryWrapper = new EntityWrapper<JianceEntity>()
            .eq("yonghu_id", jiance.getYonghuId())
            .eq("jiance_name", jiance.getJianceName())
            .eq("jiance_types", jiance.getJianceTypes())
            .eq("jiance_text", jiance.getJianceText())
//            .notIn("jiance_types", new Integer[]{102})
            ;
        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        JianceEntity jianceEntity = jianceService.selectOne(queryWrapper);
        if(jianceEntity==null){
